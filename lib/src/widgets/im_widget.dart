...
class IMWidget {
  ...
  //设置配置信息
  static final exinfoCertificate = DataPersistence.getExinfo();
  static String? get openvoice => exinfoCertificate?.openvoice;
  static String? get openvidio => exinfoCertificate?.openvidio;
  ...
  ///普通红包
  static void openenvelopeSheet(
    String label,
    String groupID,
    int nums,
    Function(int index, int num) onTapSheetItem,
  ) {
    Get.bottomSheet(
      BottomSheetView(
        itemBgColor: PageStyle.c_FFFFFF,
        items: [
          ...
          SheetItem(
            label: StrRes.envelope1,
            alignment: MainAxisAlignment.center,
            onTap: () => onTapSheetItem(1, nums),
          ),
          if (groupID != '')
            SheetItem(
              label: StrRes.envelope2,
              alignment: MainAxisAlignment.center,
              onTap: () => onTapSheetItem(2, 1),
            ),
          SheetItem(
            label: StrRes.envelope3,
            alignment: MainAxisAlignment.center,
            onTap: () => onTapSheetItem(3, nums),
          ),
        ],
      ),
      // barrierColor: Colors.transparent,
