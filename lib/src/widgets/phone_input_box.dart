...
enum InputWay { phone, email, name }
...
class PhoneInputBox extends StatelessWidget {
  ...
  String getstr(InputWay inputWay) {
    if (inputWay == InputWay.name || inputWay == InputWay.phone) {
      return StrRes.phoneNum;
    }
    return StrRes.invitecode;
  }

  TextInputType gettype(InputWay inputWay) {
    if (inputWay == InputWay.name || inputWay == InputWay.phone) {
  ...
  String gethitstr(InputWay inputWay) {
    if (inputWay == InputWay.name || inputWay == InputWay.phone) {
      return StrRes.plsInputPhone;
    }
    return StrRes.plinvitecode;
  }
  ...
  Widget _textField() => TextField(
        ...
        keyboardType: gettype(inputWay),
        textInputAction: TextInputAction.next,
        style: textStyle,
        maxLength: lenth,
        autofocus: true,
        inputFormatters: inputWay == InputWay.name
            ? [
                FilteringTextInputFormatter.allow(RegExp(r'[0-9]')),
              ]
            : null,
        decoration: InputDecoration(
          hintText: gethitstr(inputWay),
          hintStyle: hintStyle,
          isDense: true,
