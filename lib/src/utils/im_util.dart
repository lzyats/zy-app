...
class IMUtil {
  ...
  static bool isUsername(String username) {
    RegExp exp = RegExp(r'^([\u4e00-\u9fa5]|[A-Z]|[a-z]|[0-9]){6,64}$');
    return exp.hasMatch(username);
  }

  static bool isLUsername(String username) {
    RegExp exp = RegExp(r'^([\u4e00-\u9fa5]|[\.]|[A-Z]|[a-z]|[0-9]){2,64}$');
    return exp.hasMatch(username);
  }

  static bool isinvitecode(String username) {
    RegExp exp = RegExp(r'^([A-Z]|[a-z]|[0-9]){6,6}$');
    return exp.hasMatch(username);
  }
  ...
  static String? parseNotification(Message message) {
    ...
    if (message.contentType! >= 1000) {
      ...
      //print('--------notification--${message.contentType}--------${elem.detail!}');
