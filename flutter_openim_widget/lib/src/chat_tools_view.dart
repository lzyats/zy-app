...
class ChatToolsView extends StatefulWidget {
  ...
  final Function()? onEnvelope;
  final bool? envelopen;
  final String? voice;
...
class _ChatToolsViewState extends State<ChatToolsView>
  ...
  Widget _buildToolsLayout() => Container(
      ...
      padding: EdgeInsets.symmetric(horizontal: 35.w, vertical: 17.h),
      child: Wrap(
          spacing: 32.w, //Chip之间的间距大小
          runSpacing: 14.h, //行之间的间距大小
          children: <Widget>[
            Column(
              children: [
            ...
            if (widget.envelopen == true)
              Column(
                children: [
                  _toolsOption(ToolsItem(
                    label: UILocalizations.envelope,
                    style: toolsTextStyle,
                    image: _buildBtn(
                      icon: ImageUtil.toolsenvelope(),
                      onTap: widget.onEnvelope,
                    ),
                  )),
                ],
              ),
