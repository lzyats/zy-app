    ...
    try {
      switch (widget.message.contentType) {
        ...
        case MessageType.custom:
          {
            var custom = widget.message.customElem;
            var parm = custom!.toJson();
            var types = jsonDecode(parm['data']);
            if (types['customType'] == 999) {
              child = _buildCommonItemView(
                isNoborderMsg: true,
                isHintMsg: false,
                child: ChatEnvelopeView(
                    data: custom.data!,
